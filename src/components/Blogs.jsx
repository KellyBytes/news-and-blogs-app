    ...
    <div className="blogs w-full h-full flex">
      <div className="blogs-left relative w-1/2 h-full bg-[url('/images/bg.jpg')] bg-center bg-no-repeat bg-cover rounded-tl-2xl rounded-bl-2xl">
        ...
        <img
          src={userImg}
          alt="User Image"
          className="w-60 aspect-square object-cover rounded-[50%] border-[0.3rem] border-indigo-500 absolute top-1/2 -right-30 -translate-y-1/2"
        />
      </div>
      <div className="blogs-right w-1/2 h-full flex justify-center items-center relative">
        ...
        {isSubmitted && (
          <p className="submission-message text-[6rem] uppercase bg-linear-to-r from-purple-400 to-indigo-500 special-gradient">
            Post Submitted!
          </p>
        )}
        <div
          className={`blogs-right-form flex flex-col items-center gap-y-20 ${
        ...
        >
          ...
          <form className="flex flex-col gap-y-16" onSubmit={handleSubmit}>
            ...
            <textarea
              ...
              className={`text-input w-[clamp(15rem,25cqi,45rem)] bg-transparent outline-none border-b-[0.1rem] border-b-purple-400 py-8 px-0 text-neutral-200 text-[1.5rem] overflow-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] placeholder:font-comfortaa placeholder:text-[1.3rem] placeholder:text-purple-400 placeholder:opacity-50 focus:placeholder:text-transparent aspect-5/3 resize-none ${
                !isContentValid
                  ? 'border-b-[0.1rem] border-b-rose-700 placeholder:text-rose-500'
                  : ''
              }`}
        ...
        <button
          className="blogs-close-btn absolute top-12 right-10 bg-transparent border-none font-bebas text-[3rem] text-neutral-200 flex items-center cursor-pointer"
          onClick={onBack}
        >
